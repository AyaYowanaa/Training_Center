                ...
                {{-- <div id="kt_docs_jstree_basic">
                 ...
                 <ul>
                     @foreach($categories as $category)
                         <li>
                             {{ $category->name }}
  <a href="{{ route('admin.Settings.course.show', $category) }}">View</a>
                             <a href="{{ route('admin.Settings.course.edit', $category) }}">Edit</a>
                             <a href="{{ route('admin.Settings.course.destroy', $category) }}">Edit</a>
    ...
    <div class="modal fade" tabindex="-1" id="kt_modal_1">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">{{trans('TC_Setting.details')}}</h3>
        ...
        $(function () {
            $('#kt_docs_jstree_ajax').jstree({
                'core': {
                    ...
                    'data': {
                        'url': function (node) {
                            return node.id === '#' ? '{{ route("admin.Settings.course.load_roots") }}' : '{{ route("admin.Settings.course.load_child") }}';
 ...
 */
            ...
            function editCategory(id) {
                $.ajax({
                    url: '{{route('admin.Settings.course.load_edit')}}',
                    type: 'GET',
                    data: {id: id},
